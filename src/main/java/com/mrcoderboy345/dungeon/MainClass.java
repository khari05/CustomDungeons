...
public class MainClass extends PluginBase implements Listener{
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (command.getName().toLowerCase().equals("customenchant")) {
            ...
            CustomEnchant enchantment = CustomEnchantFactory.createEnchant(this,args[0],level);
        ...
        if (oldarmor.isArmor() && oldarmor.hasCompoundTag()) {
            ...
            if (oldenchants != null){
                for (int i=0; i<oldenchants.size(); i++){
                    CustomEnchantFactory.createEnchant(this, oldenchants.get(i).getString("id"), oldenchants.get(i).getInt("lvl")).offArmor(player);
                }
            }
        }
        if (newarmor.isArmor() && newarmor.hasCompoundTag()){
            ...
            if(newenchants != null){
                for (int i=0; i<newenchants.size(); i++){
                    CustomEnchantFactory.createEnchant(this,newenchants.get(i).getString("id"), newenchants.get(i).getInt("lvl")).onArmor(player);
                }
            }
        } 
    }
                ...
                if (damageearmor[i].hasCompoundTag()){
                    ...
                    if (enchants != null){
                        for (int j=0; j<enchants.size(); j++){
                            CustomEnchant enchant = CustomEnchantFactory.createEnchant(this, enchants.get(j).getString("id"), enchants.get(j).getInt("lvl"));
            ...
            if (damagerweapon.hasCompoundTag()){
                ...
                if (enchants != null){
                    for (int i=0; i<enchants.size(); i++){
                        CustomEnchantFactory.createEnchant(this, enchants.get(i).getString("id"), enchants.get(i).getInt("lvl")).onAttack(damager, ev.getDamage(),ev.getEntity());
                    }
                }
            }
        }
